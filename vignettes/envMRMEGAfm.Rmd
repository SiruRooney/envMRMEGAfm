...
gwasld=read_gwasld(gwas.loc,ld.loc,which.ld=rep(LD,each=2),cohort_name=pop,ld_name=LD,out_loc=NULL,actual.geno=TRUE)
...
- `actual.geno`: An indicator to specify whether the true cohort-level LD structure is provided. If we can obtain the real cohort-level LD structures directly or derive LD structures from the real individual-level genotype data of the involved cohorts, the argument `actual.geno` is set to `TRUE`. If we estimate LD approximation from the reference sample as a replacement, `actual.geno` is set to `FALSE`. By default `actual.geno`=TRUE.
...
envmetafm.out=env_MR_MEGA_fm(gwas.list,ld.list,which.ld=rep(names(ld.list),each=2),meta.file=envMRMEGAout,PCs=pcs,env=sexsmk,out_loc=NULL,ncores=1,collinear=0.9,pvalue_cutoff=5e-8,cred.thr=0.99,actual.geno=TRUE)
tictoc::toc()
envmetafm.out
...
tictoc::tic("Run env-MR-MEGA fine-mapping method")
metafm.out=env_MR_MEGA_fm(gwas.list,ld.list,which.ld=rep(names(ld.list),each=2),meta.file=MRMEGAout,PCs=pcs,env=NULL,out_loc=NULL,ncores=1,collinear=0.9,pvalue_cutoff=5e-8,cred.thr=0.99,actual.geno=TRUE)
tictoc::toc()
